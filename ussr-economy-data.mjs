...
export function generateMockEconomy() {
  ...
  const allCompanies = [...stateCompanies, ...privateCompanies].map((c, idx) => {
    ...
    for (let k = 0; k < 5; k++) {
      ...
      inventory[item] = Math.floor(Math.random() * 24) + 2;
    }
    // raw resources too
    const resources = ["Iron Ore", "Coal", "Oil", "Wheat", "Timber", "Fish", "Gold"];
    for (let k = 0; k < 3; k++) inventory[resources[Math.floor(Math.random() * resources.length)]] = Math.floor(Math.random() * 40) + 5;
  ...
  for (const [region, zone] of Object.entries(WORK_ZONES)) {
    ...
    const demand = Math.max(4, Math.ceil(Math.max(1, employees) * 2 + pop * 0.5));
